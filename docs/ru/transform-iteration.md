...
`Transform::memoize($iterable)` лениво кэширует пары ключ/значение, позволяя текущим и будущим потребителям начать с начала и чередоваться при одном обходе источника. Если источник бросает исключение, успешный префикс остаётся доступен, а каждый потребитель на границе получает тот же объект исключения. Повторный (реентрантный) вызов продвижения источника не поддерживается.

Кэш растёт вместе с максимально потреблённым префиксом и не освобождается. Массивы и повторно итерируемые агрегаты также оборачиваются. Используйте `tee(n)` для заранее известного числа ветвей с возможным освобождением буфера, а `memoize()` — для неизвестного числа будущих обходов. Кэширование бесконечного источника может потребовать неограниченную память.
