...
`Transform::memoize($iterable)` лениво кэширует пары ключ/значение, позволяя текущим и будущим потребителям начать с начала и чередоваться при одном обходе источника. Если источник бросает исключение, успешный префикс остаётся доступен, а каждый потребитель на границе получает тот же объект исключения.
