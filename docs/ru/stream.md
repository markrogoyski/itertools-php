...
#### Of Range
Создает ленивый поток конечной арифметической прогрессии чисел.

```Stream::ofRange(int|float|string $start, int|float|string $end, int|float $step = 1): Stream```

* Числовые строки (например, `"1"`, `"1.5"`, `"1e3"`, `"01"`) допустимы и приводятся к `int`/`float` перед итерацией. Не-числовые строки (например, `"a"`) приводят к `\InvalidArgumentException`.
* Направление выводится из соотношения `$start` и `$end`. Отрицательный `$step` допустим только при убывающем направлении.
* Выбрасывает `\InvalidArgumentException`, если какой-либо операнд не является конечным (в том числе числовые строки вроде `"1e309"`, переполняющиеся в `INF`), если `$step == 0`, если знак шага конфликтует с направлением операндов или если `abs($step) > abs($end - $start)`.
* Ленивый поток — безопасно работает с большими границами совместно с downstream-операциями вроде `limit`.
...
> Примечание: до этого релиза `Stream::ofRange` материализовал всю последовательность через `\range()` и при подаче двух числовых строк без ведущих нулей сохранял строковый тип на выходе (например, `ofRange("1", "5")` возвращал `["1", ..., "5"]`). Ленивая реализация приводит все числовые строки к `int`/`float`. Она также фиксирует числовую семантику встроенной функции `\range()` из PHP 8.3+ на всех поддерживаемых версиях PHP: шаг типа float с целым значением при целочисленных операндах теперь даёт `int` (`ofRange(1, 5, 1.0)` → `[1, 2, 3, 4, 5]`), тогда как в PHP 8.2 ранее возвращались float.
...
#### Filter With Keys
Фильтрует поток, передавая в предикат и значение, и ключ.
...
#### Flat Map With Keys
Отображение коллекции из потока функцией, учитывающей ключи, с последующим уплощением результата на 1 уровень.

Функция вызывается как `$func($value, $key, callable $self)`. Третий аргумент — сама функция, что позволяет рекурсивно уплощать вложенные коллекции. Внешние и внутренние ключи отбрасываются — результат отдаётся с автоматически сгенерированными последовательными числовыми ключами.
...
Ленивая операция: вышестоящий поток никогда не продвигается дальше тех элементов, которые были возвращены. При `$limit`, равном 0, вышестоящий поток не затрагивается вовсе.
...
#### Map With Keys
Отображение хранимой в потоке коллекции функцией, в которую передаются и значение, и ключ.
...
Ленивая операция: вышестоящий поток никогда не продвигается дальше последнего возвращённого элемента. При `$count`, равном 0, вышестоящий поток не затрагивается вовсе. Элементы, пропускаемые из-за `$start` или `$step`, всё же должны быть вычитаны, чтобы быть пропущенными.
...
#### Unzip
Рассматривает сам поток как последовательность строк и транспонирует их в столбцы — обратная операция к `zip`.

```$stream->unzip(): Stream```

Возвращает по одному массиву на каждый столбец, до ширины самой короткой строки. Внешний поток и все строки полностью потребляются при первом обращении к unzip-потоку: столбец 0 не может быть отдан, пока не известны первые ячейки всех строк.
...
##### To Mode
Возвращает список мод коллекции из потока (наиболее часто встречающихся значений).

```$stream->toMode(): array```

- Возвращает все значения с максимальной частотой в порядке первого появления.
...
##### To Percentile
Возвращает значение коллекции из потока для заданного процентиля.
...
- Процентиль `50` для любого источника возвращает ровно то же, что и `toMedian`.
- Для пустой коллекции вернет `null`.
...
##### To Quantile
Возвращает значение коллекции из потока для заданного квантиля.

```$stream->toQuantile(float $quantile): int|float|null```

- Принимает квантиль в диапазоне `[0, 1]` (например, `0.25` — первый квартиль / 25-й процентиль).
...
Возвращает стандартное отклонение значений коллекции из потока.

```$stream->toStandardDeviation(bool $sample = false): float|null```

- Квадратный корень из дисперсии. По умолчанию — стандартное отклонение генеральной совокупности; при `$sample = true` — выборочное (поправка Бесселя).
- Наследует от `toVariance` однопроходность, память `O(1)` и поведение при переполнении.
- Возвращает `null` для пустой коллекции или для выборочного стандартного отклонения единственного значения. Случаи с `null` имеют приоритет над `NAN`, как и в `toVariance`.
...
##### To Variance
Возвращает дисперсию значений коллекции из потока.

```$stream->toVariance(bool $sample = false): float|null```

- По умолчанию — дисперсия генеральной совокупности; при `$sample = true` — выборочная дисперсия (поправка Бесселя).
- Используется масштабированный онлайн-алгоритм с компенсированным текущим средним: один проход с памятью `O(1)`, поэтому поток не материализуется.
- Результат устойчив к порядку источника с точностью до ошибок округления, но **не** воспроизводится побитово: разные порядки могут отличаться на последний ulp.
- Остаётся конечным всегда, когда дисперсия представима, даже если промежуточные величины уже нет (дисперсия `[-1.4e154, 1.4e154, 0]` равна `~1.31e308`, хотя дисперсия её первых двух значений уже превышает `PHP_FLOAT_MAX`).
- **Любое неконечное значение в источнике даёт `NAN`**, так как отклонения от бесконечного среднего — это `INF - INF`. Дисперсия, которую действительно невозможно представить, возвращается как `INF`, но никогда как отрицательное число.
- Возвращает `null` для пустой коллекции или для выборочной дисперсии единственного значения. **Случаи с `null` имеют приоритет над `NAN`**: `toVariance([INF], true)` возвращает `null`, а `toVariance([INF])` — `NAN`.
...
Ленивая операция: колбэк вызывается один раз для каждого элемента в тот момент, когда нижестоящая операция вытягивает этот элемент из потока. Элементы, которые ниже по потоку не потребляются, не просматриваются.
...
Работает с потоком целиком и является нетерпеливой (eager) операцией: в отличие от поэлементного `peek`, колбэк вызывается один раз в момент вызова `peekStream()`, до выполнения каких-либо нижестоящих операций. Колбэк получает собственный поток над копией источника; потребление этого потока внутри колбэка буферизует источник, чтобы основной поток мог затем воспроизвести его.
...
Нетерпеливая (eager) операция: делегирует в `peekStream`, поэтому весь источник печатается и буферизуется в момент вызова `peekPrint()`, до выполнения каких-либо нижестоящих операций. Поэлементная ленивость `peek` здесь не действует.
...
Нетерпеливая (eager) операция: делегирует в `peekStream`, поэтому весь источник печатается и буферизуется в момент вызова `peekPrintR()`, до выполнения каких-либо нижестоящих операций. Поэлементная ленивость `peek` здесь не действует.
