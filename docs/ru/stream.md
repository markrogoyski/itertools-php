...
> Примечание: до версии v2.5.0 `Stream::ofRange` материализовал всю последовательность через `\range()` и при подаче двух числовых строк без ведущих нулей сохранял строковый тип на выходе (например, `ofRange("1", "5")` возвращал `["1", ..., "5"]`). Ленивая реализация приводит все числовые строки к `int`/`float`.
...
- Возвращает `null` для пустой коллекции или для выборочного стандартного отклонения единственного значения.
...
- Возвращает `null` для пустой коллекции или для выборочной дисперсии единственного значения.
