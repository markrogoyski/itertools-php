...
Заголовки — строки, но ключи массивов в PHP канонизируются: заголовок, являющийся канонической целочисленной строкой (`"1"`, `"2020"`), становится **целочисленным** ключом в выдаваемых строках, поэтому строка имеет тип `array<int|string, string|null>`. Это преобразование инъективно на канонических формах — заголовки, лишь похожие на числа (`"01"`, `"1.0"`, `" 1"`, `"+1"`, числа больше `PHP_INT_MAX`), остаются строковыми ключами, — поэтому ни один столбец не может быть потерян из-за совпадения ключей, а неоднозначные случаи уже отсекает проверка на дубликаты заголовков.
