...
class TraverseWideTest extends \PHPUnit\Framework\TestCase
{
    /**
    ...
    public function testArrayWithChildrenContainerKey(
        ...
        // When
        foreach (Tree::traverseWide($data, $childrenContainerKey) as $level => $item) {
            $actualLevels[] = $level;
    ...
    public function testArrayWithoutChildrenContainerKey(
        ...
        // When
        foreach (Tree::traverseWide($data) as $level => $item) {
            $actualLevels[] = $level;
            $actualIds[] = $this->getId($item);
        }
